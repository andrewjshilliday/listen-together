...
const ACCESS_TOKEN = '6JyY1rOyMYnlySjrWjmGXeepC-N9dBvKSZU0ZqWtb_PdL9mWddKnwIBVuLlwgu7Y';
...
const search = async (artistName: string, songName: string): Promise<Genius.Hit> => {
  const url = `${BASE_URL}/search?access_token=${ACCESS_TOKEN}&q=${artistName + ' ' + songName}`;
...
const getSongDetail = async (id: number): Promise<Genius.Song> => {
	const url = `${BASE_URL}/songs/${id}?access_token=${ACCESS_TOKEN}`;
	let response: Genius.GeniusResponse;
